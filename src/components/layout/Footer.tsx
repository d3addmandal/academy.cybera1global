...
function FooterMenuColumn({ menu }: { menu: NavigationMenu }) {
  return (
    <div>
      <h3 className="text-white font-semibold text-[11px] mb-2.5 uppercase tracking-widest">
        {menu.header}
      </h3>
      <ul className="space-y-1.5">
        {menu.items.map((link) => (
          <li key={link.id}>
            <Link href={link.href} className="text-[11px] text-gray-500 hover:text-red-400 transition-colors leading-snug block">
              {link.label}
            </Link>
          </li>
        ))}
    ...
    <footer className="text-gray-400" style={{ backgroundColor: "var(--color-footer-bg, #050505)" }} suppressHydrationWarning>
      <div className="w-full px-5 lg:w-[70%] lg:px-0 lg:mx-auto py-6">
        <div className={`grid grid-cols-2 ${gridCols} gap-x-8 gap-y-6 items-start`}>
          ...
          {/* Col 1: Brand — compact, no wrapping description */}
              ...
              {logoImageUrl ? (
                ...
                <>
                  ...
                  <div>
                    ...
                    <span className="text-gray-600 text-[9px] uppercase tracking-wider block">Academy</span>
                  </div>
                </>
              )}
            </Link>
            <p className="text-[11px] text-gray-600 leading-relaxed line-clamp-3">{description}</p>
          ...
          {/* Contact Us */}
          <div>
            <h3 className="text-white font-semibold text-[11px] mb-2.5 uppercase tracking-widest">Contact Us</h3>
            <ul className="space-y-1.5">
              <li className="flex items-center gap-1.5">
                <Phone style={{ width: 12, height: 12 }} className="text-red-500 flex-shrink-0" />
                <a href={`tel:${phone.replace(/\s/g, "")}`} className="text-[11px] text-gray-500 hover:text-white transition-colors">{phone}</a>
              </li>
              <li className="flex items-center gap-1.5">
                <Mail style={{ width: 12, height: 12 }} className="text-red-500 flex-shrink-0" />
                <a href={`mailto:${email}`} className="text-[11px] text-gray-500 hover:text-white transition-colors truncate">{email}</a>
              </li>
              <li className="flex items-center gap-1.5">
                <MapPin style={{ width: 12, height: 12 }} className="text-red-500 flex-shrink-0" />
                <span className="text-[11px] text-gray-500">{address}</span>
              </li>
              <li className="flex items-center gap-1.5">
                <Clock style={{ width: 12, height: 12 }} className="text-red-500 flex-shrink-0" />
              ...
              {whatsapp && (
                <li className="pt-1">
                  <a
                    ...
                    className="inline-flex items-center gap-1.5 bg-green-600 hover:bg-green-500 text-white text-[10px] font-semibold px-2.5 py-1 rounded transition-colors"
                  >
          ...
          <div>
            {lastSection === "achievements" && achievements.length > 0 ? (
              <>
                <h3 className="text-white font-semibold text-[11px] mb-2.5 uppercase tracking-widest">Achievements</h3>
                  ...
                  {achievements.map((a, i) => (
                    <div key={i} className="bg-gray-800/60 rounded p-1.5 flex items-center justify-center">
                      {a.logoUrl
                        ...
                        : <span className="text-[9px] text-gray-500 text-center leading-tight">{a.name}</span>
                      }
                    </div>
                  ))}
                </div>
              ...
              <>
                <h3 className="text-white font-semibold text-[11px] mb-2.5 uppercase tracking-widest">{newsletterTitle}</h3>
                <p className="text-[11px] text-gray-600 mb-2 leading-snug">{newsletterDesc}</p>
                <NewsletterForm />
              </>
            )}
          </div>
      ...
      <div className="border-t border-gray-800/50">
        <div className="w-full px-5 lg:w-[70%] lg:px-0 lg:mx-auto py-2.5 flex flex-col sm:flex-row items-center justify-between gap-1.5">
          <p className="text-[10px] text-gray-700">{copyright}</p>
          <div className="flex items-center gap-4 flex-wrap justify-center">
            {bottomLinks.map((link) => (
              <Link key={link.label} href={link.href} className="text-[10px] text-gray-700 hover:text-red-400 transition-colors">
                {link.label}
              </Link>
            ))}
          </div>
