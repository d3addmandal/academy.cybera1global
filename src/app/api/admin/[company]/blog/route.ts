...
export async function GET(req: NextRequest, { params }: Params) {
  ...
  return NextResponse.json({ success: true, data: blogDb.getAll(company) });
}
...
export async function POST(req: NextRequest, { params }: Params) {
  ...
  return NextResponse.json({ success: true, data: blogDb.create(company, body), message: "Post created." }, { status: 201 });
}
