...
export default function NewMenuPage() {
  ...
  const [items, setItems] = useState<MenuItem[]>([{ id: generateId(), label: "", href: "" }]);
  const [pages, setPages] = useState<PageOption[]>([]);
  const [isSaving, setIsSaving] = useState(false);

  useEffect(() => {
    fetch(`/api/admin/${company}/pages-list`)
      .then(r => r.json())
      .then(d => { if (d.success) setPages(d.data); });
  }, [company]);

  function updateItem(id: string, key: "label" | "href", val: string) {
  ...
  async function handleSave() {
    ...
    setIsSaving(true);
    try {
      const res = await fetch(`/api/admin/${company}/menus`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ header: header.trim(), items: filtered }),
      });
      const data = await res.json();
      if (data.success) { toast("Menu created!", "success"); router.push(`${base}/menus`); }
      else toast(data.error || "Failed to create menu.", "error");
