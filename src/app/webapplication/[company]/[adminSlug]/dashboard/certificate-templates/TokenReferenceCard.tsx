...
const TOKENS: { token: string; description: string }[] = [
  ...
  { token: "{{student_photo}}", description: "Student photo <img>" },
  { token: "{{course_name}}", description: "Course/programme name" },
  { token: "{{course_description}}", description: "Course description" },
  ...
  { token: "{{validity_text}}", description: "Validity note" },
  { token: "{{instructor}}", description: "Instructor name" },
  { token: "{{organization}}", description: "Organization name" },
  { token: "{{organization_logo}}", description: "Organization logo <img>" },
...
export const DEFAULT_TEMPLATE_HTML = `<div style="position:relative;width:1123px;height:794px;background:#ffffff;border:12px solid #8b0000;box-sizing:border-box;font-family:Georgia,serif;text-align:center;padding:60px;">
  ...
  <h1 style="font-size:42px;margin:20px 0 0;color:#111;">{{organization}}</h1>
  ...
  <p style="font-size:14px;color:#777;">Instructor: {{instructor}}</p>
