...
export default function FinalCTA({ cta }: Props) {
      ...
      <div className="w-full px-[2%]">
        ...
        >
          ...
          <div className="absolute inset-0 pointer-events-none">
            <div
              className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[260px] rounded-full blur-[90px]"
              style={{ background: "radial-gradient(ellipse, rgba(180,0,0,0.3) 0%, transparent 70%)" }}
            />
          </div>
            ...
            {[0.6, 0.32, 0.15].map((opacity, i) => (
              <div
                ...
                style={{
                  width: 0,
                  height: 0,
                  borderTop:    `${26 - i * 4}px solid transparent`,
                  borderBottom: `${26 - i * 4}px solid transparent`,
                  borderLeft:   `${38 - i * 5}px solid rgba(180,0,0,${opacity})`,
                }}
              />
            ))}
          </div>
          ...
          {/* Content grid */}
          <div className="relative z-10 grid lg:grid-cols-[1fr_auto_auto] gap-6 lg:gap-10 items-center">

            {/* Left: text */}
            <div>
              <p className="text-red-400 text-[10px] font-black uppercase tracking-[0.22em] mb-2">
                {eyebrow}
              </p>
              <h2 className="text-white font-black text-xl sm:text-2xl lg:text-[1.75rem] leading-snug mb-3">
                {headline}
              </h2>
            ...
            {/* Centre: buttons (stacked) */}
            <div className="flex flex-col gap-2.5 flex-shrink-0">
              <Link
                href={primary.href}
                className="inline-flex items-center justify-center gap-2 bg-red-600 text-white font-bold px-6 py-2.5 rounded-lg hover:bg-red-500 hover:shadow-[0_6px_20px_rgba(200,0,0,0.45)] transition-all text-sm whitespace-nowrap"
              >
            ...
            <div className="hidden lg:flex items-center justify-center relative w-36 h-36 flex-shrink-0">
              <div
                className="absolute inset-0 rounded-full blur-2xl opacity-70"
                style={{ background: "radial-gradient(circle, rgba(200,0,0,0.55) 0%, transparent 65%)" }}
              />
              <div className="relative flex items-center justify-center w-32 h-32">
                <svg viewBox="0 0 100 115" className="absolute w-32 h-32 opacity-20" fill="none">
                  <path d="M50 5 L90 20 L90 55 C90 80 70 100 50 110 C30 100 10 80 10 55 L10 20 Z" fill="#cc0000" />
                </svg>
                <svg viewBox="0 0 100 115" className="absolute w-24 h-24 opacity-40" fill="none">
                  <path d="M50 5 L90 20 L90 55 C90 80 70 100 50 110 C30 100 10 80 10 55 L10 20 Z" fill="#cc0000" />
                </svg>
                <svg viewBox="0 0 100 115" className="relative w-20 h-20 drop-shadow-[0_0_18px_rgba(200,0,0,0.9)]" fill="none">
                  <path d="M50 5 L90 20 L90 55 C90 80 70 100 50 110 C30 100 10 80 10 55 L10 20 Z" fill="#cc0000" />
              ...
              <div className="absolute top-2 right-0 w-6 h-px bg-red-600 opacity-50" />
              <div className="absolute top-2 right-0 w-px h-5 bg-red-600 opacity-50" />
              <div className="absolute bottom-5 left-0 w-5 h-px bg-red-600 opacity-40" />
              <div className="absolute bottom-5 left-0 w-px h-4 bg-red-600 opacity-40" />
            </div>
